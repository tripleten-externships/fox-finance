...
/**
 * Fox Finance Infrastructure CDK App
 *
 * This app creates infrastructure for the Fox Finance application across multiple environments.
 * Usage:
...
const networkStack = new NetworkStack(app, `${config.name}-Network`, {
  env,
  description: `Network infrastructure for Fox Finance ${config.name} environment`,
  tags: config.tags,
  config,
});
...
const databaseStack = new DatabaseStack(app, `${config.name}-Database`, {
  env,
  description: `Database infrastructure for Fox Finance ${config.name} environment`,
  tags: config.tags,
  config,
...
const apiStack = new ApiStack(app, `${config.name}-Api`, {
  env,
  description: `API infrastructure for Fox Finance ${config.name} environment`,
  tags: config.tags,
  config,
  vpc: networkStack.vpc,
...
const frontendStack = new FrontendStack(app, `${config.name}-Frontend`, {
  env,
  description: `Frontend infrastructure for Fox Finance ${config.name} environment`,
  tags: config.tags,
  config,
});
...
const storybookStack = new StorybookStack(app, `${config.name}-Storybook`, {
  env,
  description: `Storybook infrastructure for Fox Finance ${config.name} environment`,
  tags: config.tags,
  config,
});

// Add stack-level tags
cdk.Tags.of(app).add("Project", "fox-finance");
